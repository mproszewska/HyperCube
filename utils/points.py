...
def generate_points(config, epoch, size, normalize_points=None):
    ...
    if (
        normalize_points
        and config["target_network_input"]["normalization"]["type"] == "progressive"
    ):
        normalization_max_epoch = config["target_network_input"]["normalization"][
            "epoch"
        ]

        normalization_coef = (
            np.linspace(0, 1, normalization_max_epoch)[epoch - 1]
            if epoch <= normalization_max_epoch
            else 1
        )
        ...
        points[np.linalg.norm(points, axis=1) < normalization_coef] = (
            normalization_coef
            * (
                points[np.linalg.norm(points, axis=1) < normalization_coef].T
                / torch.from_numpy(
                    np.linalg.norm(
                        points[np.linalg.norm(points, axis=1) < normalization_coef],
                        axis=1,
